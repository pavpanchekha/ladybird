...
ErrorOr<void> DisplayListPlayer::dump_skp(DisplayList& display_list, ScrollStateSnapshotByDisplayList&& scroll_state_snapshot_by_display_list, Gfx::IntSize size, ByteString const& path)
{
    TemporaryChange change { m_scroll_state_snapshots_by_display_list, move(scroll_state_snapshot_by_display_list) };
    auto scroll_state_snapshot = m_scroll_state_snapshots_by_display_list.get(display_list).value_or({});

    SkPictureRecorder recorder;
    auto bounds = SkRect::MakeWH(size.width(), size.height());
    auto* recording_canvas = recorder.beginRecording(bounds);
    auto recording_surface = Gfx::PaintingSurface::create_for_recording(size, *recording_canvas);
    ...
    if (!picture)
        return Error::from_string_literal("Failed to record SkPicture for display list execution");

    SkFILEWStream stream(path.characters());
    if (!stream.isValid())
        return Error::from_string_literal("Failed to open SKP dump path");
